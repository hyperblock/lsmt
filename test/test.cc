...
#define RAND_RANGE (uint64_t)(rand() % (1<<20)), (uint32_t)(rand() % (1<<6) + 1) 
//生成一个1M以内，长度不超过64的线段
...
TEST(LSMT_RO, lowerbound)
{
        ...
        size_t n = 10;
        int test_count = 10;
        mappings = (struct segment_mapping *)
        ...
        struct lsmt_ro_index *index = create_memory_index(
                                                mappings, n, 0, INT64_MAX, 
                                                true);
        if (!index) {
                free(mappings);
                return;
...
{
        struct segment_mapping m4[5] = {{100,60,0}, {200,50,60}, 
                                        {300,40,110}, {400, 100, 150}, 
                                        {510,30,250}};
        struct segment_mapping m3[5] = {{90,20,0}, {175,30,20}, 
                                        {220,10,50}, {370,15,60},
                                        {800,10,75}};
...
TEST(LSMT_RO, open_files)
{
        ...
        for (off_t o = 0; o < (off_t)ro->m_vsize; o += sizeof(buf)){ //buf[1<<20], 一次读1MB
        ...
        int c_time_elapse = (t1.tv_sec - t0.tv_sec) * 1000 + (t1.tv_usec - t0.tv_usec) / 1000;
